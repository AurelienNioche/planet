...
class RSSM(base.Base):
  ...
  """
    ...
    def __init__(
            ...
            future_rnn=True, mean_only=False, min_stddev=0.1,
        ...
        self._cell = tf.compat.v1.nn.rnn_cell.GRUCell(self._belief_size)
    ...
    def _transition(self, prev_state, prev_action, zero_obs):
        ...
        for _ in range(self._num_layers):
            hidden = tf.compat.v1.layers.dense(hidden, **self._kwargs)
        belief, rnn_state = self._cell(hidden, prev_state['rnn_state'])
        if self._future_rnn:
            hidden = belief
        for _ in range(self._num_layers):
            hidden = tf.compat.v1.layers.dense(hidden, **self._kwargs)
        mean = tf.compat.v1.layers.dense(hidden, self._state_size, None)
        stddev = tf.compat.v1.layers.dense(hidden, self._state_size, tf.nn.softplus)
