...
class SSM(base.Base):
  ...
  """
    ...
    def _transition(self, prev_state, prev_action, zero_obs):
        ...
        hidden = keras.layers.Dense(inputs, **self._kwargs)
        mean = keras.layers.Dense(hidden, self._state_size, None)
        stddev = keras.layers.Dense(hidden, self._state_size, tf.nn.softplus)
