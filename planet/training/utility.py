...
def save_config(config, logdir=None):
  ...
  """
    if logdir:
        ...
        tf.compat.v1.logging.info(message)
    return config
...
def get_batch(datasets, phase, reset):
  ...
  """
    ...
    with datasets.unlocked:
        datasets.train = tf.compat.v1.data.make_one_shot_iterator(datasets.train)
        datasets.test = tf.compat.v1.data.make_one_shot_iterator(datasets.test)
    data = tf.cond(
    ...
    if 'length' not in data:
        ...
        data['length'] = (
                tf.zeros((tf.shape(example)[0],), tf.int32) +
                tf.shape(example)[1])
    return data
...
def compute_objectives(posterior, prior, target, graph, config):
    ...
    objectives = [o._replace(value=tf.reduce_mean(o.value)) for o in
                  objectives]
    return objectives
...
def apply_optimizers(objectives, trainer, config):
    ...
    objectives.append(
        Objective('main', tf.reduce_sum(losses), min, r'.*', None))
...
def simulate_episodes(
        config, params, graph, cleanups, expensive_summaries, gif_summary,
        name):
    def env_ctor():
...
def collect_initial_episodes(config):
    ...
    for name, params in items:
        ...
        if outdir not in existing:
            existing[outdir] = len(
                tf.io.gfile.glob(os.path.join(outdir, '*.npz')))
        if params.num_episodes <= existing[outdir]:
            ...
            print(
                'Collecting {} initial episodes ({}).'.format(remaining, name))
