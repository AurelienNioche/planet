...
class Run(object):
    ...
    """
        if not tf.io.gfile.exists(os.path.join(self._logdir, 'PING')):
            return None, None
        try:
            with tf.io.gfile.GFile(os.path.join(self._logdir, 'PING'),
                               'rb') as file_:
    ...
    """
        ...
        try:
            ...
            if last_worker is None:
                self._logger.info(
                    "Create directory '{}'.".format(self._logdir))
            ...
            with tf.io.gfile.GFile(os.path.join(self._logdir, 'PING'),
                               'wb') as file_:
                pickle.dump((self._worker_name, datetime.datetime.utcnow()),
                            file_)
        except (EOFError, IOError, tf.errors.NotFoundError):
            raise WorkerConflict
    ...
    def _create_logger(self):
        ...
        for name in 'debug info warning'.split():
            methods[name] = lambda unused_self, message: getattr(tf.compat.v1.logging,
                                                                 name)(
                'Worker {} run {}: {}'.format(self._worker_name, run_name,
                                              message))
