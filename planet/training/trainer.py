...
class Trainer(object):
  ...
  """
    ...
    """
        ...
        self._report = tf.compat.v1.placeholder(tf.bool, name='report')
    ...
    def add_saver(
            self, include=r'.*', exclude=r'.^', logdir=None, load=True,
            save=True,
            checkpoint=None):
    ...
    """
        ...
        saver = tf.compat.v1.train.Saver(variables, keep_checkpoint_every_n_hours=2)
        if load:
        ...
        if checkpoint is None and self._config.checkpoint_to_load:
            self._checkpoints.append(
                os.path.join(self._logdirs[-1],
                             self._config.checkpoint_to_load))
        else:
    ...
    """
        ...
        with sess:
            ...
            while True:
                ...
                summary, mean_score, global_step = sess.run(phase.op,
                                                            phase.feed)
                ...
                if summary and phase.writer:
                    ...
                    longest_phase = max(
                        phase_.steps for phase_ in self._phases)
