...
class CustomOptimizer(object):
    ...
    def __init__(
        ...
        self._variables = filter_variables_lib.filter_variables(include,
                                                                exclude)
    ...
    def maybe_minimize(self, condition, loss):
        ...
        if self._debug:
            ...
            assertion = lambda: tf.assert_greater(grad_norm, 0.0,
                                                  message=message)
    ...
    def minimize(self, loss):
        with tf.name_scope('optimizer_{}'.format(self._name)):
            if self._debug:
                loss = tf.compat.v1.check_numerics(loss, '{}_loss'.format(self._name))
    ...
    def summarize(self, grad_norm):
        ...
        with tf.name_scope('optimizer_{}'.format(self._name)):
            ...
            if self._clipping:
                ...
                summaries.append(
                    tf.compat.v1.summary.scalar('clipped_gradient_norm', clipped))
