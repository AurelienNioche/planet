...
def simulate(
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        return_mean = tf.reduce_mean(input_tensor=return_)
...
def simulate_step(batch_env, algo, log=True, reset=False):
    ...
    def _define_begin_episode(agent_indices):
        ...
        with tf.control_dependencies(reset_ops):
            return algo.begin_episode(
                agent_indices), update_score, update_length
    ...
    """
        ...
        with tf.control_dependencies([batch_env.step(action)]):
            ...
            inc_length = length_var.assign_add(
        ...
        with tf.control_dependencies([add_score, inc_length]):
            ...
            experience_summary = algo.experience(
                ...
                batch_env.done,
                batch_env.observ)
            summary = tf.compat.v1.summary.merge([step_summary, experience_summary])
    ...
    def _define_summaries():
        ...
        score_summary = tf.cond(
            ...
            lambda: tf.compat.v1.summary.scalar('mean_score',
                                                mean_score.clear()), str)
        length_summary = tf.cond(
            tf.logical_and(log, tf.cast(mean_length.count, tf.bool)),
            lambda: tf.compat.v1.summary.scalar('mean_length',
                                                mean_length.clear()), str)
        return tf.compat.v1.summary.merge([score_summary, length_summary])

    with tf.name_scope('simulate'):
        log = tf.convert_to_tensor(value=log)
        reset = tf.convert_to_tensor(value=reset)
        ...
        begin_episode, score, length = tf.cond(
            tf.cast(tf.shape(input=agent_indices)[0], tf.bool),
